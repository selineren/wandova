...
final class CountryBoundaryService {
    ...
    private var cachedRenderOverlays: [String: [MKOverlay]]?
    private let cacheLock = NSLock()

    private init() {}

    /// Full-resolution overlays. Used for tap hit-testing, where border
    /// accuracy matters. Backed by the shared single decode of
    /// world_countries.geojson.
    func getCountryOverlays() -> [String: [MKOverlay]] {
        WorldGeoJSONStore.shared.load().overlaysByCountry
    }

    /// Simplified overlays (generated by tools/simplify-borders.sh). Used
    /// for everything that draws polygons — ~44K vertices instead of ~548K,
    /// which keeps VectorKit's overlay re-tessellation cheap.
    func getRenderOverlays() -> [String: [MKOverlay]] {
        cacheLock.lock()
        defer { cacheLock.unlock() }

        if let cached = cachedRenderOverlays {
            return cached
        }

        let overlays = buildOverlays(from: WorldGeoJSONStore.decodeObjects(resource: "world_countries_simplified"))
        cachedRenderOverlays = overlays
        return overlays
    }

    /// Builds the per-country overlay dictionary from decoded GeoJSON.
    /// Stateless; called by WorldGeoJSONStore for the full-resolution file
    /// and by getRenderOverlays for the simplified one.
    func buildOverlays(from objects: [MKGeoJSONObject]) -> [String: [MKOverlay]] {
